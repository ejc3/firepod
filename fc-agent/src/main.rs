...
async fn main() -> Result<()> {
    ...
    if let Some(image_dir) = &plan.image_dir {
        ...
        // Debug: test network connectivity before pull
        eprintln!("[fc-agent] testing network connectivity...");
        ...
        // Test 2: Ping gateway (get from route table)
        if let Ok(route_output) = std::process::Command::new("ip")
            .args(["route", "show", "default"])
            .output()
        {
            let route_str = String::from_utf8_lossy(&route_output.stdout);
            // Parse "default via X.X.X.X dev ethN" to get gateway
            if let Some(gateway) = route_str.split_whitespace().nth(2) {
                eprintln!("[fc-agent] pinging gateway: {}", gateway);
                if let Ok(output) = std::process::Command::new("ping")
                    .args(["-c", "1", "-W", "2", gateway])
                    .output()
                {
                    eprintln!(
                        "[fc-agent] ping gateway: {}",
                        if output.status.success() {
                            "OK"
                        } else {
                            "FAILED"
        ...
        eprintln!("[fc-agent] testing DNS resolution for public.ecr.aws...");
        ...
        // Test 4: Test outbound connectivity to known IP
        eprintln!("[fc-agent] testing outbound connectivity to 8.8.8.8...");
        if let Ok(output) = std::process::Command::new("ping")
            .args(["-c", "1", "-W", "3", "8.8.8.8"])
            .output()
        {
            eprintln!(
                "[fc-agent] ping 8.8.8.8: {}",
                if output.status.success() {
                    "OK"
                } else {
                    "FAILED"
