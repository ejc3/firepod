...
///
/// Requires kernel with FUSE remap_file_range support (inception kernel with patch).
/// Run `test_fuse_copy_file_range_vm` to test inside a VM with the patched kernel.
#[test]
fn test_copy_file_range() {
    ...
    let test_data = b"Hello, copy_file_range! This tests reflink support.";
    let src_path = mount.join("source.txt");
    let dst_path = mount.join("dest.txt");

    fs::write(&src_path, test_data).expect("write source file");
    fs::write(&dst_path, b"").expect("create empty destination");

    // Open both files
    let src_file = fs::File::open(&src_path).expect("open source");
    ...
    assert!(
        result >= 0,
        "copy_file_range failed: {} (requires kernel with FUSE remap patch - run test_fuse_copy_file_range_vm instead)",
    ...
    let copied = fs::read(&dst_path).expect("read destination");
    assert_eq!(&copied, test_data, "copied data should match");
    ...
    let src_file = fs::File::open(&src_path).expect("reopen source");

    let offset = 7usize; // "Hello, " is 7 bytes
    let partial_len = test_data.len() - offset;
    let mut off_in: libc::off64_t = offset as i64;
    let mut off_out: libc::off64_t = test_data.len() as i64; // Append
    ...
    assert!(
        result >= 0,
        "partial copy_file_range failed: {}",
        std::io::Error::last_os_error()
    );
    assert_eq!(result as usize, partial_len, "should copy partial bytes");

    drop(src_file);
    drop(dst_file);

    // Verify appended data
    let final_data = fs::read(&dst_path).expect("read final destination");
    assert_eq!(
        final_data.len(),
        test_data.len() + partial_len,
        "final size should be original + partial"
    );
    assert_eq!(
        &final_data[..test_data.len()],
        test_data,
        "first part should be original"
    );
    assert_eq!(
        &final_data[test_data.len()..],
        &test_data[offset..],
        "appended part should match"
    );

    // Cleanup
    fs::remove_file(&src_path).expect("remove source");
    fs::remove_file(&dst_path).expect("remove destination");
    drop(fuse);
    cleanup(&data_dir, &mount_dir);
}
